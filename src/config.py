...
# 나머지를 잊는 사고가 난다. 실제로 그랬다.
#
# 값 선택 근거는 **홀드아웃**이다. 개발셋 argmax 를 그대로 쓰면 안 된다는 것을
# 이 값 자체가 보여줬다 (한 번 0.69 를 넣었다가 되돌렸다):
#
#   thr    dev      hold      오거부(hold)
#   0.47   35/54    41/53     1/36
#   0.66   46/54    47/53     3/36     <- 채택. 모든 축에서 최선
#   0.69   49/54    41/53     9/36     <- dev 단일점 스파이크
#
# 0.69 는 dev 에서 혼자 튄다(이웃 0.67/0.68/0.70 은 전부 46). 홀드아웃에서는 41/53 으로
# **아무것도 안 고친 0.47 과 같고**, 오거부만 1건에서 9건으로 늘었다.
# 함정 거부는 0.69 가 10/12 로 1건 앞서지만, 그 1건을 위해 답할 수 있는 질문 6건을
# 더 거부한다. CLAUDE.md 8-2 가 경계하라는 "과잉 거부"가 정확히 이 교환이다.
BM25_THRESHOLD = 0.66   # 개발셋 46/54, 홀드아웃 47/53, gap -0.035 (과적합 징후 없음)
...
ABSTAIN_THRESHOLD = {
    ...
    # openai 는 dense/ensemble 둘 다 미측정이다. 스윕으로 확정할 것.
    "openai":  {"dense": 0.38, "bm25": BM25_THRESHOLD, "ensemble": 0.34},
    # hash 는 오프라인 폴백. dense/ensemble 미측정이고 발표에 쓰지 않는다.
